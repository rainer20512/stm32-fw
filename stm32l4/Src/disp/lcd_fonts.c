...
#include "../src/disp/fonts/lcd/digits_32px.c"
#include "../src/disp/fonts/lcd/digits_24px.c"
#include "../src/disp/fonts/lcd/font_proportional_16px.c"
#include "../src/disp/fonts/lcd/symbols_16px.c"
#include "../src/disp/fonts/lcd/font_fixed_8px.c"
#include "../src/disp/fonts/lcd/font_proportional_8px.c"
 ...
 *****************************************************************************/
static const uint8_t *font_data(FONT_P font) 
{
    return font->data;
}
 ...
 */
static const uint8_t *font_widthtable(FONT_P font) 
{
 ...
 *****************************************************************************/
static uint8_t font_get_height_bytes(FONT_P font) 
{
  uint8_t t = font->height;
 ...
 *****************************************************************************/
static uint8_t font_get_add_space(FONT_P font, char character) 
{
  (void)character;
  const uint8_t *type = font_widthtable(font);
  if ( type ) //if there is a width table, then it's a proportional font
    return 1;
  else
    return 0;
}
 ...
 *****************************************************************************/
static int16_t font_get_char_number(FONT_P font, char character) 
{
 ...
 *****************************************************************************/
static uint8_t font_get_char_width(FONT_P font, char character) 
{
  const uint8_t *table = font_widthtable(font);
  if (table)
    return *(table+font_get_char_number(font,character));
  else
    return font->width;
}
 ...
 *****************************************************************************/
const uint8_t *font_get_char_position(FONT_P font, char character) 
{
  uint16_t ret         = 0;
  int16_t  charnum_ret = font_get_char_number(font, character);
  uint8_t  charnum     = charnum_ret;
  const uint8_t *base  = font_widthtable(font);
 ...
 *****************************************************************************/
static unsigned char double_bits(uint8_t part, char c) 
{
  uint8_t t = 0;
  if (part) c = c>>4;
  if (c & 0x08) t  = 0xC0;
...
/******************************************************************************
 * Returns the pixel width of a character of given fond and style ---RHB added
 *****************************************************************************/
uint8_t lcd_get_charwidth(FONT_P font, uint8_t style, char character) 
{
...
uint8_t lcd_put_char(FONT_P font, uint8_t style, char character) 
{
  ...
  //load information about character
   ...
   uint8_t free_space    = font_get_add_space(font,character)*spc;
   const uint8_t *posn   = font_get_char_position(font,character);
  ...
  do {
    for(i=(row>>hc); i<char_width*font_height; i+=font_height) {
      tmp = *(posn+i);
      if(row == char_final_height-1) tmp |= ul;
      if(hc)                         tmp = double_bits((row&1),tmp);
      if(inv)                        tmp = ~tmp;
      LCD_WRITE(tmp);
      if(wc)  LCD_WRITE(tmp);
    }
...
/******************************************************************************
 * Returns the length ( in pixels ) of a string ---RHB Added
 ******************************************************************************/
uint16_t lcd_get_strlen(FONT_P font, uint8_t style, const char* str) 
{
  unsigned char t;
  uint16_t length = 0;
  while((t = *str++))
    length += lcd_get_charwidth(font,style,t);
  return length;
}
 ...
 *****************************************************************************/
uint16_t lcd_put_string_length(FONT_P font, uint8_t style, const char *str, uint8_t length) 
{
  unsigned char t;
 ...
 *****************************************************************************/
uint16_t lcd_put_string_xy(FONT_P font, uint8_t style, const char *str,uint8_t page, uint8_t col) 
{
 ...
 */   
uint16_t lcd_putstr(const char* str) 
{
 ...
 * given position
 */   
uint16_t lcd_putstr_xy(const char *str, uint8_t page, uint8_t col) 
{
