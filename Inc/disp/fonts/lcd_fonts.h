...
#define LCD_MOVE(x,y)       dogm_move_xy((x),(y))     //relative cursor movement
#define LCD_MOVE_TO(x,y)    dogm_moveto_xy((x),(y))   //absolute cursor movement
#define LCD_WRITE(x)        dogm_data((x))            //write data to display
#define LCD_CLEAR(x,y)      dogm_clear_area((x),(y))  //clear part of the lcd, sized x times y

//Functions to read the current position as provided by the LCD library
#define LCD_CURRENT_COL()   dogm_get_position_column()
#define LCD_CURRENT_PAGE()  dogm_get_position_page()
#define LCD_WIDTH()         DOGM_WIDTH
...
struct font_info {
    ...
    const uint8_t *widthtable;  //Pointer to the table holding character widths (NULL for monospaced fonts)
    const uint8_t *data;        //Pointer to data arrray
    };



#endif /* ifndef __LCD_FONTS_H */
