...
/******************************************************************************
 * CM4 Stub for remote (B)DMA channel registration
 *****************************************************************************/
void MSGD_DoRemoteDmaRegistration(const HW_DmaType* dmaData, uint8_t dmaGroup)
{
    /* 
     * Register a DMA channel in use by CM4 core on CM7 registrar.
     * infos are stored in RAM, 
     */
    AMP_DctBuf_Ptr->msg6.dmaDesc    = dmaData;
    AMP_DctBuf_Ptr->msg6.dmaGroup   = dmaGroup;
    AMP_DctBuf_Ptr->msg6.opMode     = MSGTYPE6_MODE_REG;
    AMP_DctBuf_Ptr->msg_id          = MSGTYPE_REGISTER_DMA_CHANNEL;
    AMP_DctBuf_Ptr->msg_status      = MSGSTATUS_CM4_TO_CM7_ACTIVE;
    Ipc_CM4_SendDirect();
}
...
void MSGD_DoRemoteDmaUnRegistration(DMA_HandleTypeDef *hdma, uint8_t dmaGroup)
{
     ...
     * infos are stored in RAM, 
     */
    AMP_DctBuf_Ptr->msg6.hdma       = hdma;
    AMP_DctBuf_Ptr->msg6.dmaGroup   = dmaGroup;
    AMP_DctBuf_Ptr->msg6.opMode     = MSGTYPE6_MODE_UNREG;
    AMP_DctBuf_Ptr->msg_id          = MSGTYPE_REGISTER_DMA_CHANNEL;
    AMP_DctBuf_Ptr->msg_status      = MSGSTATUS_CM4_TO_CM7_ACTIVE;
    Ipc_CM4_SendDirect();
}
...
DMA_HandleTypeDef *MSGD_WaitForRemoteDmaRegisterOp(void)
{
    ...
    /* Check for the Timeout */
        if ((HAL_GetTick() - tickstart) > DIRECTMSG_WAIT_TIMEOUT) {
            DEBUG_PUTS("DirectMessage Wait #6 timed out!");
            return NULL;
        }
    }
    /* Rewrite the Channel information to DMA-Handle */
    DMA_HandleTypeDef *ret = AMP_DctBuf_Ptr->msg6.responseHandle;
    if ( ret  ) ret->Instance = AMP_DctBuf_Ptr->msg6.responseChannel;
    return ret;
}
